  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix">
        <span
          class="username"
          :class="{ selected: this.payWay == 'ONLINE' }"
        ...
        <span
          ...
          :class="{ selected: this.payWay == 'ONDELIVERY' }"
          @click="paymentWay('ONDELIVERY')"
          >货到付款</span
        >
      </div>
      <div class="line"></div>
      ...
      <div class="detail">
        ...
        >
          ...
          <li class="order-skuName order-center">
            <p>
              {{ order.skuName }}
            </p>
            <h4 class="order-h4">7天无理由退货</h4>
          </li>
          <li class="order-center">
            <h3>¥{{ order.orderPrice }}.00</h3>
          </li>
          <li class="order-center">X{{ order.skuNum }}</li>
          <li class="order-center">有货</li>
        </ul>
      </div>
      <div class="bbs">
        <h5>买家留言：</h5>
...
export default {
  ...
  computed: {
    ...
    //将来提交订单最终选中地址
    userDefaultAddress() {
      //find：查找数组里满足条件的元素【第一个】，作为返回结果。
      //filter：在这里也可以，但是filter返回新数组,数组内index为0是一个我们需要的对象。
      // return this.addressInfo.filter(item =>item.isDefault==1)[0]||{};

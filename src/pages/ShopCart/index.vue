...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      /**
       *  type:为了区分是点击- + input框  哪一个
       *  disNum形参：+变化量（+1）   -变化量（-1）   input最终的个数（并不是变化量）
       *  cart：区分点击哪个产品（为了产品id）
       *  向服务器发请求，修改数量
       */
      switch (type) {
        //加号
        ...
        case "minus":
          ...
          //否则产品个数小于等于1，传递给服务器0（不变）
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        //
        case "change":
          //用户输入进来的最终量，如果是非法（带有汉字|字符|负数），带给服务器disNum为0
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0;
          } else {
            //正常情况（小数：取证），带给服务器变化的量 （用户输入进来的量-产品起始个数）

        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转是从A到B，这里加入购物车进行的路由跳转前发请求，把购买产品信息通过请求形式通知服务器，服务器进行存储 -->
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果skuInfo是一个空对象，那么skuInfo的skuImageList是undefined，所以skuImageList必须至少是一个空数组
  ...
  methods: {
    ...
    async addShopCar() {
      //1.发请求---通知服务器将产品存到数据库
      /**
       * 派发action向服务器发请求，判断加入购物车是成功还是失败，进行相应操作。
       * this.$store.dispatch('AddOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum});
       * 上面这行代码：调用vuex仓库的AddOrUpdateShopCart，这个方法加async，返回一定是一个Promise
       * 要么是Promise成功|要么是Promise失败
       * 因此addShopCar也是异步函数
       */
      try {
        ...
        //2.服务器存储成功---路由跳转
        /**
         * 2.1路由跳转时还需要将产品信息带给AddCartSuccess组件
         * 下面这个方法可以但是不采用！
         * this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}});
         * 使用本地存储存skuId，而query只存skuNum
         * 简单数据skuNum用query传递过去
         * 产品数据skuInfo用会话存储（不持久化，会话结束消失）
         * 注意：本地存储|会话存储，一般只存字符串
         */ 
